...
class DataBaseThursday extends SQLiteOpenHelper {

    private Context context;
    private static final String DATABASE_NAME = "Thursday.db";
    ...
    DataBaseThursday(@Nullable Context context) {
    ...
    void addBook(String title, String author){
        ...
        cv.put(COLUMN_SUB, title);
        cv.put(COLUMN_BOOK, author);
        long result = db.insert(MONDAY_TABLE,null, cv);
        if(result == -1){
    ...
    void updateData(String row_id, String title, String author){
        ...
        long result = db.update(MONDAY_TABLE, cv, "_id=?", new String[]{row_id});
        if(result == -1){
            Toast.makeText(context, "Failed", Toast.LENGTH_SHORT).show();
        }else {
    ...
    void deleteAllData(Context context){
        context.deleteDatabase("Thursday.db");
    }
